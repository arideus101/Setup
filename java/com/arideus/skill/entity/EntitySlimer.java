...
public class EntitySlimer extends EntityLiving implements IMob {
	...
	public EntitySlimer(World worldIn) {
		super(worldIn);
		this.moveHelper = new EntitySlimer.SlimeMoveHelper();
		this.tasks.addTask(1, new EntitySlimer.AISlimeFloat());
		this.tasks.addTask(2, new EntitySlimer.AISlimeAttack());
		this.tasks.addTask(3, new EntitySlimer.AISlimeFaceRandom());
		this.tasks.addTask(5, new EntitySlimer.AISlimeHop());
	...
	protected void setSlimeSize(int p_70799_1_) {
		...
		this.getEntityAttribute(SharedMonsterAttributes.maxHealth)
				.setBaseValue((double) ((p_70799_1_ + 2) * (p_70799_1_ + 2)));
		this.getEntityAttribute(SharedMonsterAttributes.movementSpeed)
				.setBaseValue((double) (0.2F + 0.1F * (float) (p_70799_1_ + 2)));
	...
	public void onUpdate() {
		...
		List list = worldObj.getEntitiesWithinAABB(EntitySlimer.class,
				this.getEntityBoundingBox().expand(0.2, 0.2, 0.2));
		if (list.size() > 4 && this.rand.nextInt(4) == 0 && !this.isDead) {
			int ls = 0;
			for (int i = 1; i < list.size(); i++) {
				if (((EntitySlimer) list.get(i)).getSlimeSize() > ((EntitySlimer) list.get(ls)).getSlimeSize()) {
					ls = i;
				}
			}
			for (int i = 0; i < list.size(); i++) {
				if (i != ls)
					((EntitySlimer) list.get(i)).setDead();
			}
			((EntitySlimer) list.get(ls)).setSlimeSize(((EntitySlimer) list.get(0)).getSlimeSize() + 2);
		}
	...
	protected EntitySlimer createInstance() {
		return new EntitySlimer(this.worldObj);
	}
	...
	public void setDead() {
		...
		if (!this.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0F) {
			...
			for (int k = 0; k < j; ++k) {
				...
				EntitySlimer entityslime = this.createInstance();
	...
	protected Item getDropItem() {
		return this.getSlimeSize() == 1 ? Items.ender_pearl : null;
	}
	...
	public boolean getCanSpawnHere() {
		Chunk chunk = this.worldObj.getChunkFromBlockCoords(
	...
	class AISlimeAttack extends EntityAIBase {
		private EntitySlimer field_179466_a = EntitySlimer.this;
		...
		public boolean continueExecuting() {
			...
			return entitylivingbase == null ? false
					: (!entitylivingbase.isEntityAlive() ? false : --this.field_179465_b > 0);
		}
		...
		public void updateTask() {
			...
			((EntitySlimer.SlimeMoveHelper) this.field_179466_a.getMoveHelper())
	...
	class AISlimeFaceRandom extends EntityAIBase {
		private EntitySlimer field_179461_a = EntitySlimer.this;
		...
		public boolean shouldExecute() {
			return this.field_179461_a.getAttackTarget() == null && (this.field_179461_a.onGround
					|| this.field_179461_a.isInWater() || this.field_179461_a.isInLava());
		}
		...
		public void updateTask() {
			...
			((EntitySlimer.SlimeMoveHelper) this.field_179461_a.getMoveHelper()).func_179920_a(this.field_179459_b,
					false);
		}
	}

	class AISlimeFloat extends EntityAIBase {
		private EntitySlimer field_179457_a = EntitySlimer.this;
		private static final String __OBFID = "CL_00002201";

		public AISlimeFloat() {
			this.setMutexBits(5);
			((PathNavigateGround) EntitySlimer.this.getNavigator()).func_179693_d(true);
		}
		...
		public void updateTask() {
			...
			((EntitySlimer.SlimeMoveHelper) this.field_179457_a.getMoveHelper()).func_179921_a(1.2D);
		}
	}

	class AISlimeHop extends EntityAIBase {
		private EntitySlimer field_179458_a = EntitySlimer.this;
		 ...
		 */
		public void updateTask() {
			((EntitySlimer.SlimeMoveHelper) this.field_179458_a.getMoveHelper()).func_179921_a(1.0D);
		}
	}
	...
	class SlimeMoveHelper extends EntityMoveHelper {
		...
		private EntitySlimer field_179925_i = EntitySlimer.this;
		private boolean field_179923_j;
		private static final String __OBFID = "CL_00002199";

		public SlimeMoveHelper() {
			super(EntitySlimer.this);
		}
