...
public class EntityArrowDragonSkin extends EntityArrow {
	private int xTile = -1;
	private int yTile = -1;
	private int zTile = -1;
	...
	public EntityArrowDragonSkin(World worldIn) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	}

	public EntityArrowDragonSkin(World worldIn, double x, double y, double z) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	...
	public EntityArrowDragonSkin(World worldIn, EntityLivingBase shooter, EntityLivingBase p_i1755_3_, float p_i1755_4_,
			float p_i1755_5_) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
	...
	public EntityArrowDragonSkin(World worldIn, EntityLivingBase shooter, float p_i1756_3_) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = shooter;
	...
	public void onUpdate() {
		List slist = worldObj.getEntitiesWithinAABB(EntityLiving.class, getEntityBoundingBox().expand(10, 10, 10));
		if (slist.size() > 0) {
			for (int i = 1; i < 10; i++) {
				slist = worldObj.getEntitiesWithinAABB(EntityLiving.class, getEntityBoundingBox().expand(i, i, i));
				if (slist.size() > 0) {
					double x1 = ((EntityLivingBase) slist.get(0)).posX;
					double y1 = ((EntityLivingBase) slist.get(0)).posY;
					double z1 = ((EntityLivingBase) slist.get(0)).posZ;
					double x2 = this.posX;
					double y2 = this.posY;
					double z2 = this.posZ;
					double distance = (Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2) + Math.pow(z1 - z2, 2)));
					this.addVelocity((x1 - x2) / distance*i, (y1 - y2) / distance*i, (z1 - z2) / distance*i);
					this.fallDistance = 0;
					continue;
				}
			}
		...
		// Vec3(this.posX, this.posY, this.posZ))) {
		// this.inGround = true;
		// }
		// }
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (this.shootingEntity == null) {
						damagesource = Main.causeArrowDamageDragonSkin(this, this);
					} else {
						damagesource = Main.causeArrowDamageDragonSkin(this, this.shootingEntity);
					}
					...
					// this.motionX = (double) ((float)
					// (movingobjectposition.hitVec.xCoord - this.posX));
					// this.motionY = (double) ((float)
					// (movingobjectposition.hitVec.yCoord - this.posY));
					// this.motionZ = (double) ((float)
					// (movingobjectposition.hitVec.zCoord - this.posZ));
			...
			if (this.getIsCritical()) {
				for (i = 0; i < 4; ++i) {
					this.worldObj.spawnParticle(EnumParticleTypes.CRIT_MAGIC,
							...
							this.posZ + this.motionZ * (double) i / 4.0D, 0, 0, 0, new int[0]);
				}
			}
