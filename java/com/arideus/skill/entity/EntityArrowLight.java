...
public class EntityArrowLight extends EntityArrow
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityArrowLight(World worldIn)
    {
        super(worldIn);
    ...
    public EntityArrowLight(World worldIn, double x, double y, double z)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityArrowLight(World worldIn, EntityLivingBase shooter, EntityLivingBase p_i1755_3_, float p_i1755_4_, float p_i1755_5_)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityArrowLight(World worldIn, EntityLivingBase shooter, float p_i1756_3_)
    {
        super(worldIn);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = shooter;
    ...
    public void setDead()
    {
    	if(worldObj.getBlockState(new BlockPos(posX, posY, posZ))==Blocks.glowstone.getDefaultState())
    	worldObj.setBlockState(new BlockPos(posX, posY, posZ), Blocks.air.getDefaultState());
        this.isDead = true;
    }
    /**
    ...
    public void onUpdate()
    {
        ...
        if (this.inGround)
        {
            ...
            {
                ++this.ticksInGround;
                if(worldObj.isAirBlock(new BlockPos(posX, posY, posZ)))
                worldObj.setBlockState(new BlockPos(posX, posY, posZ), Blocks.glowstone.getDefaultState());
        ...
        else
        {
            ...
            if (movingobjectposition != null && movingobjectposition.entityHit != null && movingobjectposition.entityHit instanceof EntityPlayer && movingobjectposition.entityHit != this.shootingEntity)
            {
            ...
            if (movingobjectposition != null)
            {
                if (movingobjectposition.entityHit != null&&movingobjectposition.entityHit!= this.shootingEntity)
                {
                    ...
                    {
                        damagesource = Main.causeArrowDamageLight(this, this);
                    }
                    else
                    {
                    	
                        damagesource = Main.causeArrowDamageLight(this, this.shootingEntity);
                    }
                    ...
                    if (this.shootingEntity != movingobjectposition.entityHit)
            ...
            if (this.getIsCritical())
            {
                for (i = 0; i < 4; ++i)
                {
                    this.worldObj.spawnParticle(EnumParticleTypes.FLAME, this.posX + this.motionX * (double)i / 4.0D, this.posY + this.motionY * (double)i / 4.0D, this.posZ + this.motionZ * (double)i / 4.0D, 0,0,0, new int[0]);
                }
            }
