...
public class EntityHook extends Entity implements IProjectile {
	...
	public EntityHook(World worldIn, EntityLivingBase shooter, float p_i1756_3_) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = shooter;
	...
	public EntityHook(World worldIn, EntityLivingBase shooter,
			float p_i1756_3_, Item Ammo, boolean Gravity) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = shooter;
	...
	public void setThrowableHeading(double p_70186_1_, double p_70186_3_,
		...
		this.prevRotationYaw = this.rotationYaw = (float) (Math.atan2(
				p_70186_1_, p_70186_5_) * 180.0D / Math.PI);
		this.prevRotationPitch = this.rotationPitch = (float) (Math.atan2(
				p_70186_3_, (double) f3) * 180.0D / Math.PI);
	...
	public void setVelocity(double p_70016_1_, double p_70016_3_,
		...
		if (this.prevRotationPitch == 0.0F && this.prevRotationYaw == 0.0F) {
			...
			this.prevRotationYaw = this.rotationYaw = (float) (Math.atan2(
					p_70016_1_, p_70016_5_) * 180.0D / Math.PI);
			this.prevRotationPitch = this.rotationPitch = (float) (Math.atan2(
					p_70016_3_, (double) f) * 180.0D / Math.PI);
	...
	public void onUpdate() {
		...
		if (!this.Gravity) {
			...
			if (Math.sqrt(Math.pow(this.motionX, 2) + Math.pow(this.motionY, 2)
					+ Math.pow(this.motionZ, 2)) < 16)
				this.setVelocity(this.motionX * 1.2, this.motionY * 1.2,
						this.motionZ * 1.2);
		}
		...
		if (block.getMaterial() != Material.air) {
			...
			if (axisalignedbb != null
					&& axisalignedbb.isVecInside(new Vec3(this.posX, this.posY,
							this.posZ))) {
				this.inGround = true;
			}
		}
		...
		if (this.inGround) {
			...
			if (block == this.inTile && j == this.inData) {
					...
					if(distance>2);
					this.shootingEntity.addVelocity((x1 - x2)/distance/4, (y1 - y2)/distance/4,
							(z1 - z2)/distance/4);
			...
			for (i = 0; i < list.size(); ++i) {
				...
				if (entity1.canBeCollidedWith()
					...
					AxisAlignedBB axisalignedbb1 = entity1
							.getEntityBoundingBox().expand((double) f1,
									(double) f1, (double) f1);
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (this.shootingEntity == null) {
						damagesource = Main.causeBulletDamageHook(this,
								this);
					} else {
						damagesource = Main.causeBulletDamageHook(this,
								this.shootingEntity);
					...
					f3 = MathHelper.sqrt_double(this.motionX * this.motionX
							+ this.motionY * this.motionY + this.motionZ
							* this.motionZ);
	...
	public void readEntityFromNBT(NBTTagCompound tagCompund) {
		...
		if (tagCompund.hasKey("inTile", 8)) {
			this.inTile = Block
					.getBlockFromName(tagCompund.getString("inTile"));
		} else {
			this.inTile = Block
	...
	public void setDead() {// if(ammo!=null)this.worldObj.spawnEntityInWorld(new
		...
		if (ammo != null) {
			if (this.shootingEntity instanceof EntityPlayer) {
				((EntityPlayer) this.shootingEntity).inventory
						.addItemStackToInventory(new ItemStack(ammo, 1));
			} else {
				this.worldObj.spawnEntityInWorld(new EntityItem(this.worldObj,
						this.shootingEntity.posX, this.shootingEntity.posY,
						this.shootingEntity.posZ, new ItemStack(ammo)));
			}
		}
		this.isDead = true;
	}
