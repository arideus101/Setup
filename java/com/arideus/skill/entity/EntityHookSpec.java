...
public class EntityHookSpec extends EntityHook {
	private int xTile = -1;
	private int yTile = -1;
	private int zTile = -1;
	...
	public EntityHookSpec(World worldIn) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	}

	public EntityHookSpec(World worldIn, double x, double y, double z) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	...
	public EntityHookSpec(World worldIn, EntityLivingBase shooter, EntityLivingBase p_i1755_3_, float p_i1755_4_,
			float p_i1755_5_) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
	...
	public EntityHookSpec(World worldIn, EntityLivingBase shooter, float p_i1756_3_) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = shooter;
	...
	public EntityHookSpec(World worldIn, EntityLivingBase shooter, float p_i1756_3_, Item Ammo, boolean Gravity) {
		super(worldIn);
		this.renderDistanceWeight = 10.0D;
		this.shootingEntity = shooter;
	...
	public void setThrowableHeading(double p_70186_1_, double p_70186_3_, double p_70186_5_, float p_70186_7_,
		...
		// p_70186_1_ += this.rand.nextGaussian()
		// * (double) (this.rand.nextBoolean() ? -1 : 1)
		// * 0.007499999832361937D * (double) p_70186_8_;
		// p_70186_3_ += this.rand.nextGaussian()
		// * (double) (this.rand.nextBoolean() ? -1 : 1)
		// * 0.007499999832361937D * (double) p_70186_8_;
		// p_70186_5_ += this.rand.nextGaussian()
		// * (double) (this.rand.nextBoolean() ? -1 : 1)
		// * 0.007499999832361937D * (double) p_70186_8_;
	...
	public void onUpdate() {
		...
		if (!this.Gravity) {
			...
			if (Math.sqrt(Math.pow(this.motionX, 2) + Math.pow(this.motionY, 2) + Math.pow(this.motionZ, 2)) < 16)
				this.setVelocity(this.motionX * 1.2, this.motionY * 1.2, this.motionZ * 1.2);
		}
		...
		if (this.inGround) {
			...
			if (block == this.inTile && j == this.inData) {
				...
				double y1 = this.posY;
				double z1 = this.posZ;
				double x2 = this.shootingEntity.posX;
				double y2 = this.shootingEntity.posY;
				double z2 = this.shootingEntity.posZ;
				double distance = (Math.sqrt(Math.pow(x1 - x2, 2) + Math.pow(y1 - y2, 2) + Math.pow(z1 - z2, 2)));
				this.shootingEntity.addVelocity((x1 - x2) / distance, (y1 - y2) / distance, (z1 - z2) / distance);
				this.shootingEntity.fallDistance = 0;
			...
			if (movingobjectposition != null) {
				if (movingobjectposition.entityHit != null) {
					...
					if (this.shootingEntity == null) {
						damagesource = Main.causeBulletDamageHook(this, this);
					} else {
						damagesource = Main.causeBulletDamageHook(this, this.shootingEntity);
					}
					...
					if (movingobjectposition.entityHit.attackEntityFrom(damagesource, (float) k)) {
						...
						double y1 = this.shootingEntity.posY;
						double z1 = this.shootingEntity.posZ;
						double x2 = movingobjectposition.entityHit.posX;
						double y2 = movingobjectposition.entityHit.posY;
						double z2 = movingobjectposition.entityHit.posZ;
						movingobjectposition.entityHit.addVelocity((x1 - x2) / 4, (y1 - y2) / 4, (z1 - z2) / 4);
			...
			if (this.getIsCritical()) {
				for (i = 0; i < 4; ++i) {
					this.worldObj.spawnParticle(EnumParticleTypes.CRIT_MAGIC,
							this.posX + this.motionX * (double) i / 4.0D, this.posY + this.motionY * (double) i / 4.0D,
	...
	public void setDead() {// if(ammo!=null)this.worldObj.spawnEntityInWorld(new
		// EntityItem(this.worldObj, this.posX, this.posY,
		// this.posZ, new ItemStack(ammo)));
		if (ammo != null) {
			if (this.shootingEntity instanceof EntityPlayer) {
				((EntityPlayer) this.shootingEntity).inventory.addItemStackToInventory(new ItemStack(ammo, 1));
			} else {
				this.worldObj.spawnEntityInWorld(new EntityItem(this.worldObj, this.shootingEntity.posX,
						this.shootingEntity.posY, this.shootingEntity.posZ, new ItemStack(ammo)));
			}
		}
		this.isDead = true;
	}
